...
import javax.ws.rs.WebApplicationException;
    ...
    /**
 * Clase que implementa el recurso "reservas/{id}/usuario".
 *
 * @author ISIS2603
 * @version 1.0
 */
...
public class ReservaUsuarioResource 
{
    ...
    @Inject
    private ReservaUsuarioLogic reservaLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias.

    @Inject
    private UsuarioLogic usuarioLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias.

    /**
     * Remplaza la instancia de usuario asociada a un reserva.
     *
     ...
     * @return JSON {@link ReservaDetailDTO} - El arreglo de reservas guardado en el usuario
     * @throws WebApplicationException {@link WebApplicationExceptionMapper} -
     * Error de lógica que se genera cuando no se encuentra el usuario o la reserva.
     */
    @PUT
    public ReservaDetailDTO replaceUsuario(@PathParam("reservasId") Long reservasId, UsuarioDTO usuario) {
        LOGGER.log(Level.INFO, "ReservaResource replaceUsuario: input: reservasId{0} , Usuario:{1}", new Object[]{reservasId, usuario.toString()});
        ...
        if (usuarioLogic.getUsuario(usuario.getId()) == null) {
            throw new WebApplicationException("El recurso /reservas/" + usuario.getId() + " no existe.", 404);
        }
